...
function hasPermission(roles: string[], route: RouteRecordRaw) {
  if (route.meta && route.meta.roles) {
    ...
    return roles.some((role) => {
      if (route.meta?.roles !== undefined) return (route.meta.roles as string[]).includes(role);
    });
  }
  return false;
}
...
function filterAccessRoutes(routes: RouteRecordRaw[], roles: string[]) {
  ...
  routes.forEach((route) => {
    ...
    if (hasPermission(roles, tempRoute)) {
      ...
      if (tempRoute.children) tempRoute.children = filterAccessRoutes(tempRoute.children, roles);
